...
const stationColumns = require('../models/station.json')
const queries = require('../queries')

const parseFloatNull = (val) => {
  return isNaN(parseFloat(val)) ? null : parseFloat(val)
}

const parseIntNull = (val, radix) => {
  ...
  try {
    ...
    stations.forEach(station => {
      dbStations.push({
        ...
        rloi_id: parseIntNull(station.RLOI_ID, 10),
        station_type: station.Station_Type,
        post_process: station.Post_Process,
        subtract: parseFloatNull(station.Subtract),
        ...
        x_coord_actual: parseIntNull(station.X_coord_Actual, 10),
        y_coord_actual: parseIntNull(station.Y_Coord_Actual, 10),
        actual_ngr: station.Actual_NGR,
        x_coord_display: parseIntNull(station.X_coord_Display, 10),
        y_coord_display: parseIntNull(station.Y_coord_Display, 10),
        site_max: parseFloatNull(station.Site_Max),
    ...
    // Clear out stations
    await db.query(queries.deleteStations)
    ...
    for (let i = 0; i <= stationsFactor; i++) {
      await db.query(stationTable.insert(dbStations.slice(i * 500, (i * 500) + 500)).toQuery())
    }
